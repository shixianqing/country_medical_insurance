...
class CustomDownloaderMiddleware(object):
    ...
    def process_request(self, request, spider):
        ...
        try:
            ...
            print('超时')
            self.writeFile(url, fileName=self.exception_path)
            return request
    ...
    def process_response(self, request, response, spider):
        ...
        if response.status != 200:
            self.writeFile(url=response.url,fileName=self.exception_path)
            return request
        return response

    def process_exception(self, request, exception, spider):
        print("{}出现异常".format(request.url))
        self.writeFile(url=request.url,fileName=self.exception_path)
        return request
